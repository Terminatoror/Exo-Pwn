...
f_config="/handshakes5/5.conf"
f_db="/handshakes5/db"
d_handshakes="/handshakes5/handshakes"
...
hc_run(){
	...
	do
		...
		while read -r line; do bssid=$(echo $line | awk -F ',' '{print $1}'); essid=$(echo $line | awk -F',' '{print $14}'); channel=$(echo $line | awk -F',' '{print $4}'); echo $bssid,$essid,$channel; done < /tmp/hc_out-01.csv | grep -iE "([0-9A-F]{2}[:-]){5}([0-9A-F]{2}), [-a-zA-Z0-9_ !]+, ([0-9]{1,2})" > /tmp/hc_stations5.tmp

		#echo "[*] Clearing temp files"
		rm /tmp/hc_out5*

		readarray stations < /tmp/hc_stations5.tmp

		mkdir /tmp/hc_captures5 &> /dev/null
		mkdir /tmp/hc_handshakes5 &> /dev/null
		...
		do
            ...
            # Capture handshake
            timeout --foreground 10s airodump-ng -w /tmp/hc_captures5/"$essid" --output-format pcap --bssid "$bssid" --channel "$channel" "$interface" &> /dev/null
            ...
            # Convert to .hccapx
            cap2hccapx "/tmp/hc_captures5/$essid.pcap" "/tmp/hc_handshakes5/$essid.hccapx" &> /dev/null
            ...
            mv "/tmp/hc_captures5/$essid-01.cap" "$d_handshakes/$unique_cap" &> /dev/null

            # Check if handshake is valid
            hashflag=$(wlanhcxinfo -i "/tmp/hc_handshakes5/$essid.hccapx" 2>&1)
            if [[ $hashflag == *"0 records loaded"* ]]; then
                # Remove invalid .hccapx and move .pcap to destination
                rm "/tmp/hc_handshakes5/$essid.hccapx" "$d_handshakes/$essid.hccapx" &> /dev/null
                mv "/tmp/hc_captures5/$essid-01.pcap" "$d_handshakes/$essid-01.pcap" &> /dev/null
            else
                # Move valid .hccapx to destination with unique name
                mv "/tmp/hc_handshakes5/$essid.hccapx" "$d_handshakes/$unique_hccapx" &> /dev/null
                ((hs_count++))
		...
		then
			...
			echo -en "\033[1A"
			echo -en "\033[2K"
			echo -en "\rLast scan: ${YELLOW}`ls /tmp/hc_handshakes5/ | wc -l` new handshakes captured${NC}"
			echo -en "\033[1B"
			echo -en "\r"
		fi
	done
