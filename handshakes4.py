...
class HandshakeCounter(plugins.Plugin):
    ...
    def on_ui_setup(self, ui):
        ...
        ui.add_element('Head4', LabeledValue(color=BLACK, label='Head 4: ', value=str(self.handshake_count),
                position=(ui.width() / 2, ui.height() - 46),
                label_font=fonts.Bold, text_font=fonts.Medium))

    def on_ui_update(self, ui):
        # Check if secondary_pwn.service is running
        if self.is_service_running('second_pwn3.service'):
            ui.set('Head4', str(self.handshake_count))
        else:
            ui.set('Head4', 'X')
    ...
    def periodic_update(self):
        while self.running:
            self.update_counter()
            current_service_status = self.is_service_running('second_pwn3.service')
    ...
    def update_counter(self):
        handshake_dir = '/handshakes4/handshakes/'
        try:
            count = len([
