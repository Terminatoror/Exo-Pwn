...
class HandshakeCounter(plugins.Plugin):
    ...
    def on_ui_setup(self, ui):
        ...
        ui.add_element('Head2', LabeledValue(color=BLACK, label='Head 2: ', value=str(self.handshake_count),
                position=(ui.width() / 2, ui.height() - 46),
                label_font=fonts.Bold, text_font=fonts.Medium))

    def on_ui_update(self, ui):
        # Check if secondary_pwn.service is running
        if self.is_service_running('secondary_pwn.service'):
            ui.set('Head2', str(self.handshake_count))
        else:
            # If service is not running, display 'X'
            ui.set('Head2', 'X')

    def periodic_update(self):
        while self.running:
            self.update_counter()
            current_service_status = self.is_service_running('secondary_pwn.service')
    ...
    def update_counter(self):
        handshake_dir = '/handshakes2/handshakes/'
        try:
            count = len([
