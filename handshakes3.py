...
class HandshakeCounter(plugins.Plugin):
    ...
    def on_ui_setup(self, ui):
        ...
        ui.add_element('Head3', LabeledValue(color=BLACK, label='Head 3: ', value=str(self.handshake_count),
                position=(ui.width() / 2, ui.height() - 46),
                label_font=fonts.Bold, text_font=fonts.Medium))

    def on_ui_update(self, ui):
        # Check if secondary_pwn.service is running
        if self.is_service_running('second_pwn2.service'):
            ui.set('Head3', str(self.handshake_count))
        else:
            ui.set('Head3', 'X')
    ...
    def periodic_update(self):
        while self.running:
            self.update_counter()
            current_service_status = self.is_service_running('second_pwn2.service')
    ...
    def update_counter(self):
        handshake_dir = '/handshakes3/handshakes/'
        try:
            count = len([
