...
class HandshakeCounter(plugins.Plugin):
    ...
    def on_ui_setup(self, ui):
        ...
        ui.add_element('Head5', LabeledValue(color=BLACK, label='Head 5: ', value=str(self.handshake_count),
                position=(ui.width() / 2, ui.height() - 46),
                label_font=fonts.Bold, text_font=fonts.Medium))

    def on_ui_update(self, ui):
        if self.is_service_running('second_pwn4.service'):
            ui.set('Head5', str(self.handshake_count))
        else:
            ui.set('Head5', 'X')
    ...
    def periodic_update(self):
        while self.running:
            self.update_counter()
            current_service_status = self.is_service_running('second_pw4.service')
    ...
    def update_counter(self):
        handshake_dir = '/handshakes5/handshakes/'
        try:
            count = len([
